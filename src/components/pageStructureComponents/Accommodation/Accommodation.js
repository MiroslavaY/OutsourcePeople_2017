            ...
            accommodation.map((el, i)=> {
              return (
                <div className="accommodation-items__item" key={el.name}>
                  <div className="accommodation-items__item__icon">{i + 1}</div>
                  <p className="accommodation-items__item__name">{el.name}</p>
                  <a className="accommodation-items__item__description" href={el.href}>Подробнее</a>
                </div>
              )
            })
          }
